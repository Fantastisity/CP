...
bool comp[(int)(1e6 + 1)];
int fact[(int)(1e6 + 1)], prefix[(int)(1e6 + 1)][11];
void solve() {
	for (int i = 2, l = (int)1e6; i <= l; ++i) {
		if (!comp[i]) {
			fact[i] = 1;
			for (ll j = i << 1; j <= l; j += i) {
				comp[j] = 1;
				++fact[j];
			}
		}
	...
	prefix[2][1] = 1;
	for (int i = 3, l = (int)1e6; i <= l; ++i) {
		prefix[i][fact[i]] = 1;
		FE(j, 0, 10) prefix[i][j] += prefix[i - 1][j];
	}
}
...
int main() {
    ...
    while (n--) {
    	...
    	if (a == b) cout << (fact[a] == c), nl;
    	else cout << (prefix[b][c] - prefix[a - 1][c]), nl;
    }
    return 0;
}
