...
int Gcd(int a, int b) {
	if (!a || !b) return a | b;
	unsigned shift = __builtin_ctz(a | b); // determine common factor
	a >>= __builtin_ctz(a); // remove factor 2 from a
	do {
		b >>= __builtin_ctz(b); // remove factor 2 from b
		if (a > b) swap(a, b);
		b -= a; // gcd(a, b) = gcd(b, a - b) since both a & b are odd numbers
	} while (b);
	return a << shift; // apply common factor
}
